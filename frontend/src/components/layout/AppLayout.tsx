...
export function AppLayout() {
  ...
  return (
    <div className="h-screen overflow-hidden bg-zinc-950 text-zinc-200">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-black/50 px-2 pb-[calc(env(safe-area-inset-bottom,0)+0.5rem)] pt-2 backdrop-blur-md lg:hidden">
          ...
          {moduleNavItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    'min-w-[78px] flex-1 flex-col items-center justify-center rounded-xl border border-transparent px-2 py-2 text-[11px] uppercase tracking-wide text-zinc-400 transition-all duration-200 flex',
